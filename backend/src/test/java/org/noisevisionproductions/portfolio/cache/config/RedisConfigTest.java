package org.noisevisionproductions.portfolio.cache.config;
...
class RedisConfigTest {
    ...
    @Test
    void cacheConfiguration_ShouldCreateCacheConfiguration() {
        RedisCacheConfiguration configuration = redisConfig.cacheConfiguration();
        RedisCacheConfiguration defaultConfig = RedisCacheConfiguration.defaultCacheConfig()
                .entryTtl(Duration.ofMillis(60));

        assertNotNull(configuration);
        assertEquals(defaultConfig.getTtlFunction(), configuration.getTtlFunction());
